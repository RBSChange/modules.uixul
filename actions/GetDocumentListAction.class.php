...
class uixul_GetDocumentListAction extends f_action_BaseAction
{
	...
	private function retrieveDocumentsInTree($request)
	{
		...
		if (!$request->hasParameter('cmpref')
			|| (!is_numeric($request->getParameter('cmpref')) && !is_array($request->getParameter('cmpref'))))
		{
		...
		{
			$list = $request->getParameter('cmpref');
			if (!is_array($list))
			{
				$list = array($list);
			}
		...
		if ($request->hasParameter('includeFolder')
			&& (intval($request->getParameter('includeFolder')) == 1 || $request->getParameter('includeFolder') == "true"))
		{
			$includeFolder = true;
		}
		else
	...
	protected function getDocumentList($parentId, $baseClass, $deep, $level = 0, $includeFolder = false, $icon = null)
	{
		...
		foreach ($documents as $document)
		{
			...
			if ($classIndex != -1 && ($document->getPersistentModel()->useCorrection() == false || !$document->getCorrectionofid())
				...
				&& (substr($document->getDocumentModelName(), -12) != '/preferences'))
			{
				...
				$info = array('document' => $document, 'parent' => $parentId, 'level' => $level, 'isFolder' => $isFolder);
		...
		if ($deep === true)
		{
			...
			{
				$leveledList = array_merge($leveledList, $this->getDocumentList($subFolder, $componentType, $deep, $level + 1));
			}
		}
		return $leveledList;
	}
