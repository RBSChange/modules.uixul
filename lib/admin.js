...
function getServerCacheVersion()
{
    ...
    req.open('GET', '{UIHOST}/cache/www/cacheversion.txt?dummy='+(new Date()).getTime(), false);
    req.send(null);
...
function help()
{
    ...
    if (moduleName)
    {
        wToolkit.dialog('uixul', 'ShowHelp', {wemod: moduleName}, {width: 650}, true, true);
    }
    else
    {
        wToolkit.dialog('uixul', 'ShowHelp', null, {width: 650}, true, true);
    }
}
...
function changePassword()
{
	...
	var url = controller.makeUrl('users', 'ChangePsswd');
	window.open(url, 'changePsswd', 'width=600, height=600, modal=yes, dialog=yes, titlebar=no, alwaysRaised=yes, close=yes, menubar=no, toolbar=no, location=no, resizable=yes, scrollbars=yes, status=no"');
}
...
function editLocale()
{
	...
	if (moduleName)
	{
		wToolkit.dialog('uixul', 'EditLocale', {wemod: moduleName}, {width: 980, height: 650}, true, true);
	}
	else
	{
		wToolkit.dialog('uixul', 'EditLocale', null, {width: 980, height: 650}, true, true);
	}
}
...
//Global Administartion function

function loadModule(moduleName, perspectiveName, locateDocumentId, onLoadAction)
{
	if (moduleName === "uixul")
	{
		document.getElementById("contextual-help").setAttribute("disabled", "true");
	}
	else
	{
		document.getElementById("contextual-help").removeAttribute("disabled");
	}
	...
	else
	{
		...
		else if (onLoadAction)
		{
			...
			else if (typeof(onLoadAction) == "string" && typeof(module[onLoadAction]) == "function")
			{
				try
				{
					module[onLoadAction]();
