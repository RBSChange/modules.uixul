...
function getServerCacheVersion()
{
	...
	if (result.status === 'OK')
	{
		return result.contents.cacheVersion;
	}
    return "";
}
...
function clearCache()
{
	...
	try
	{
		...
		var xchromeservice = Components.classes["@mozilla.org/network/protocol;1?name=xchrome"].getService(Components.interfaces.nsIProtocolHandler).wrappedJSObject;
		if ('getExtensionBySpec' in xchromeservice) {
			var ext = xchromeservice.getExtensionBySpec(window.location.href.split('#')[0]);
			if (ext) {
				ext.clearCache();
			}
		}
...
{
	wCore.debug('clearImageCache');
	try {
		var cacheService = Components.classes["@mozilla.org/network/cache-service;1"].getService(Components.interfaces.nsICacheService);
		var appService =  Components.classes["@mozilla.org/xre/app-info;1"].getService(Components.interfaces.nsIXULAppInfo);
		cacheService.evictEntries(Components.interfaces.nsICache.STORE_ON_DISK);

		var imgService = null;
		if (appService.version >= '18.0') {
			imgService = Components.classes["@mozilla.org/image/tools;1"]
				.getService(Components.interfaces.imgITools)
				.getImgCacheForDocument(null);

		} else {
			imgService = Components.classes["@mozilla.org/image/cache;1"]
				.getService(Components.interfaces.imgICache)
				.imgCacheService;
		}
		imgService.clearCache(true); // true=chrome, false=content
		imgService.clearCache(false); // true=chrome, false=content
	}
	catch(exception) {
		wCore.error('clearImageCache', [], exception);
	}
}
