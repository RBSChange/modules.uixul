...
wToolkit.startUploadFile = function (file, target, sender)
{
    try
    {
		...
		try
		{
            ...
            stateCol.setAttribute('src', Context.UIBASEURL + '/changeicons/small/clock.png');
...
wToolkit.successUploadFile = function (fileName, sender)
{
    try
    {
    	...
    	progressMeter.nextSibling.setAttribute('src', Context.UIBASEURL + '/changeicons/small/check.png');
    }
    catch (e)
    {
...
wToolkit.setNotificationMessage = function(message, type, timeout)
{
	...
	{
		wCore.info(message+" "+type);
	}
	
	return;
}

// this is a deprecated function with no replacement
